...
import { Heart, CarFront, Layout, ArrowLeft, Home } from "lucide-react";
...
const Header = async ({ isAdminPage = false }) => {
  ...
  return (
    <header className="fixed top-0 w-full bg-white z-50 border-b shadow-sm h-20 grid grid-cols-2 md:grid-cols-3">
      {/* Logo Section */}
      <div className="pl-5 sm:pl-12 flex items-center">
        <Link href={isAdminPage ? "/admin" : "/"} className="flex items-center space-x-2">
          ...
          />
          {isAdminPage && (
            <span className="text-sm sm:text-xl font-bold text-black">Admin</span>
          )}
        </Link>
      </div>

      {/* WheelDeal Title (only on desktop) */}
      <div className="hidden md:flex justify-center items-center px-6 py-4">
      ...
      {/* Action Buttons */}
      <div className="flex items-center space-x-2 sm:space-x-4 justify-end pr-4 sm:pr-6">
        {isAdminPage ? (
          <Link href="/">
